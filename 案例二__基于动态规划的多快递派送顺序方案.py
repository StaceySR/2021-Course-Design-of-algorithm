...
if __name__ == '__main__':
    # 随机生成10个投放地点的两两之间距离的邻接矩阵
    D = matrix_creator(10)
    print("快递投放点邻接矩阵为：")
    print(D)
    client = []
    print("请输入需要投放到哪些地点（首先输入原点，以“#”结束）： ")
    while True:
        res = input()
        if res == '#':
            break
    ...
    while len(lists) > 0:
        ...
        if start == 0:
            print("物流点出发", "--到-->", str(client[next_node])+"号地点")
        elif next_node == 0:
            print(str(client[start])+"号地点", "--到-->", "返回物流点")
        else:
            print(str(client[start])+"号地点", "--到-->", str(client[next_node])+"号地点")
        start = next_node